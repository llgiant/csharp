...
class Rect
{
    ...
    public bool ContainsRect(Rect rect)
    {
        ...
        // if ( rect.Top >= Bottom || rect.Left >= Right || !ContainsPoint(rect.Point)) { return false; }

        if (rect.Square < Square && ContainsPoint(rect.Point) && rect.Right <= Right && rect.Bottom <= Bottom) { return true; }
        return false;
    ...
    public Rect Union(Rect rect)
    {
        ...
        return new Rect(comRight - comLeft, comBottom - comTop, comLeft, comRight);
    }
    public bool IsIntersect(Rect rect)
    {
        if (ContainsPoint(new Point(rect.X, rect.Y))) { return true; }
        else if (ContainsPoint(rect.X + rect.Width, rect.Y)) { return true; }
        else if (ContainsPoint(rect.X, rect.Y + rect.Height)) { return true; }
        else if (ContainsPoint(rect.X + rect.Width, rect.Y + rect.Height)) { return true; }
        else { return false; };
