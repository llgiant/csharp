...
class Template
{
    ...
    //public Template(string pattern, Dictionary<string, validateProc> fieldsValidators, BaseHandler handler)
    //{
    ...
    public Dictionary<string, string> Match(Uri incomingURL)
    {
        ...
        do
        {
            ...
            if (caret == urlLocalPath.Length)
            {
                if (_list[index + 1] == null) { return validatedFillers; }
                else { return null; }
            } //true
    ...
    public static bool validate_imageFileName(string value)
    {
        ...
        if (!allSymbols.Contains(value)) { throw new Exception("Не соответствующий формат у заполнителя"); }
 ...
 extension -> только буквы, от 3 до 5 символов
    ...
    public bool Match(Uri incomingURL)
    {
        ...
        while (index < _list.Count)
        {
            ...
            //    /app/style/images/imageFileName.jpg.jpg
            //    /app/style/images/{imageFileName}.{extension}
