...
class Program
{
	...
	static void Main()
	{
		...
		//Выписсать построчно все предложения из текста.
		string strSentance = "";
		startIndex = 0;
		for (int index = 0; index < text.Length; index++)
		{
			if (text[index] == '.' || text[index] == '!' || text[index] == '?')
			{
				...
				{
					if (!char.IsLetter(text[index + 1]) && !char.IsPunctuation(text[index + 1]) && char.IsUpper(text[index + 2]))
					{
						if (strSentance.Length > 0) { strSentance += "\n"; }
						strSentance += text.Substring(startIndex, index - startIndex + 1);
						startIndex = index + 2;
					}
				}
				else
		...
		Console.WriteLine($"Cлова начинающиеся на букdу \"Р\": { startsWithP}");
		...
		Console.WriteLine();
		Console.WriteLine("Выписсать построчно все предлоожения из текста:");
		Console.Write(strSentance);
